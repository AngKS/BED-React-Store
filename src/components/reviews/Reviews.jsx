...
function Reviews() {
    ...
    useEffect(() => {
        ...
        axios.get(`https://react-game-marketplace.herokuapp.com/reviews/${gameKey}`, { headers: {'Access-Control-Allow-Origin': 'http://bed-react-store.netlify.app/'}}).then(res => {
    ...
    let sendReview = () => {
        let header = { 'authorization': 'Bearer ' + localStorage.getItem('token'), 'Access-Control-Allow-Origin': 'http://bed-react-store.netlify.app/' }
        axios({
            method : "POST",
            headers: header,
